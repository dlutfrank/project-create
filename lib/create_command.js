...
class Command {
  async run(cwd, argv) {
    ...
    this.variables = await this.processVariable(this.targetDir, this.templateDir);
    if (!this.variables) {
      return Promise.reject('process variable failed');
    }
    await this.processFiles(this.targetDir, this.templateDir, this.variables);
      ...
      return await inquirer.prompt(
        keys.map((key) => {
          ...
          return {
            ...
            name: key,
            message: question.desc || question.message,
  ...
  async processFiles(targetDir, templateDir, variables) {
    this.log(`target: ${targetDir}, template: ${templateDir}`);
    this.log(`variables: ${JSON.stringify(variables)}`);
    const src = path.join(templateDir, 'template');
    const files = globby.sync('**/*', { cwd: src, dot: true });
    files.forEach((file) => {
      const from = path.join(src, file);
      const to = path.join(targetDir, this.replaceVariable(file, variables));
      const content = fs.readFileSync(from);
      const result = istextorbinary.isTextSync(from, content)
        ? this.replaceVariable(content.toString(), variables) : content;
